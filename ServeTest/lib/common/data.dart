...
final statelessList = <ContainerInfo>[
  ...
  ContainerInfo('CardWidget', '卡片组件，基于Material组件实现，用于将单个组件卡片画', '/card'),
  ContainerInfo(
      'VisiblityWidget', '显示隐藏组件，控制一个组件显示或者隐藏，可设置隐藏后的占位组件。', '/visiblity'),
  ...
  ContainerInfo('IconWidget', '用于图标显示的组件。', '/icon'),
  ContainerInfo(
  ...
  ContainerInfo(
      ...
      '/myprederredsize'),
  ContainerInfo('CupertinoWidget', 'iOS风格组件', '/cupertino'),
];
...
final statefulList = <ContainerInfo>[
  ...
  ContainerInfo('Material', 'Material 风格组件的核心，可指定颜色、阴影颜色、形状等属性。', '/material'),
  ContainerInfo(
      'MaterialApp', 'Material 应用的顶级组件，包含路由生成器、主题、语言、主页等属性。', '/materialapp'),
  ContainerInfo(
      'WillPopScope',
      '返回拦截，当一个页面中有WillPopScope组件时，在页面返回时会触发回调，决定是否返回，当页面返回时会触发回调，决定是否返回。可用于二次确认退出的场景。',
      '/willpopscope'),
  ContainerInfo(
  ...
  ContainerInfo(
      ...
      '/futurebuilder'),
  ContainerInfo('Overlay', '可以将组件在全应肌进行悬浮显示，能够添加或移除组件，它们有独立管理的栈。', '/overlay'),
  ContainerInfo(
      'Steeper', '步骤条组件，可指定一步步的操作，可自定义步骤的内容，确认和返回的按钮以及步骤排列的方向。', '/steeper'),
  ContainerInfo('Checkbox', '复选组件、单选组件、切换组件。', '/checkbox'),
  ContainerInfo(
      'Slider', '滑块组件，可以在指定的最大值和最小值之间拖动选择。可指定颜色、分段数及显示的接收进度条变化回调。', '/slider'),
